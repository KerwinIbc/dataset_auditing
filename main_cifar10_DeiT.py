# ============================================================
# change_main_methodB_lambda_budget_shadow_then_target.py
#
# ✅ Merged version:
#   - Training is handled by train_target_DeiT.py (target+shadow)
#   - Deterministic train/query subsets returned for prior/budget/MI
#   - cVAE deterministic training set built by loading the SAME split indices
#   - MI(train/query/gen) uses compute_information_plane.py
#     where compute_MI_full_dataset is the "equivalent counting" version
#
# (Your inversion, dual-ascent, gen-MI logic kept)
# ============================================================
...
from train_DeiT import train_target_model
from train_cVAE import train_cVAE
from compuet_MI_DeiT import (
...
def ib_sgld_blackbox_nes_multichain(
    ...
    opt_g = optim.Adam(generator.parameters(), lr=lr_g) if update_generator else None
...
def main():
    ...
    # -------------------------
    # 1) Train target/shadow + get deterministic train/query sets
    # -------------------------
    target_model, shadow_model, train_set, query_set = train_target_model(
        device=device,
    ...
    # infer classes + build query_set_vae from same split indices
    ...
    # load SAME split indices to build cVAE dataset ([-1,1])
    split_path = f"./checkpoints/split_indices_{DATASET_NAME}_seed{SEED}.npz"
    z = np.load(split_path)
    query_idx = z["query_idx"].astype(int).tolist()

    vae_tf = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,0.5,0.5), (0.5,0.5,0.5)),
    ])
    full_vae = ds_cls(root="./data", train=True, download=False, transform=vae_tf)
    query_set_vae = Subset(full_vae, query_idx)

    print(f"[Reuse Split] query_set_vae size = {len(query_set_vae)}")

    # -------------------------
    # 2) Train generator on query_set_vae
    # -------------------------
    ...
    # -------------------------
    eval_subset = list(range(10)) if CLASSES == 10 else list(range(20))
    ...
    ensure_dir("results")
    with open(f"results/methodB_lambda_dual_history_{DATASET_NAME}.txt", "w") as f:
        for r in hist:
            f.write(str(r) + "\n")
    ...
    imgs_gen, labels_gen = run_inversion_experiment(
        method_name=f"Method-B Final ({DATASET_NAME})",
        ...
        device=device,
        save_dir=f"results/blackbox_methodB_{DATASET_NAME}",
        z_dim=128,
        total_steps=3500,
        burn_in=800,
        num_chains=64,
    ...
    # -------------------------
    # 7) Information Plane (MI)
    #     - train/query: FULL dataset single point (counting version)
    #     - gen: sampled multi points (kept)
    # -------------------------
    print("\n📊 Computing TRAIN MI (FULL, counting) ...")
    train_XT, train_TY = compute_MI_full_dataset(
        target_model, train_set, device=device, batch_size=256, num_workers=2, num_label=CLASSES
    )
    train_XT, train_TY = np.array(train_XT), np.array(train_TY)

    print("\n📊 Computing QUERY MI (FULL, counting) ...")
    test_XT, test_TY = compute_MI_full_dataset(
        target_model, query_set, device=device, batch_size=256, num_workers=2, num_label=CLASSES
    )
    test_XT, test_TY = np.array(test_XT), np.array(test_TY)

    print("\n📊 Computing GENERATED MI (balanced sampling, kept) ...")
    ...
    ax.scatter([train_XT], [train_TY], s=160, label="Train (80%, full)")
    ax.scatter([test_XT], [test_TY], s=160, label="Query (20%, full)")
    ax.scatter(gen_XT, gen_TY, s=60, marker="^", label="Gen (multi)")
    ...
    ell = Ellipse(
        xy=(cx, cy), width=2*a, height=2*b, angle=0.0,
        fill=False, linewidth=2, linestyle="--"
    )
    ax.add_patch(ell)
    ax.scatter([cx], [cy], marker="x", s=80, label=f"Gen mean (a={a:.4f}, b={b:.4f})")

    ax.set_xlabel("I(X;T)")
    ax.set_ylabel("I(T;Y)")
    ax.set_title(f"Information Plane: {DATASET_NAME} DeiT (Train/Query full + Gen)")
    ax.grid(True)
    ax.legend()

    out_png = f"results/info_plane_methodB_{DATASET_NAME}_merged_counting.png"
