# ============================================================
# change_main_methodB_lambda_budget_shadow_then_target_CIFAR10_VGG16.py
#
# CIFAR-10 + VGG16(target/shadow) version (10 classes)
#
...
#
# ✅ Plot: axis-aligned ellipse centered at gen mean:
#   a = max |x - cx|,  b = max |y - cy|
# ============================================================
...
# ============================================================
# 1) VGG16 for CIFAR-10 (32x32) helper
# ============================================================
def build_vgg16_cifar(num_classes=10):
...
# ============================================================
# 2) Train target/shadow VGG16 on CIFAR-10 with 80/20 split
#    Returns:
#      target_model, shadow_model, train_set(80%), query_set(20%)
# ============================================================
def train_target_model_vgg16_cifar10(
    device="cuda",
    seed=0,
    data_root="./data",
    train_epochs_target=30,
    train_epochs_shadow=20,
    batch_size=128,
    lr=0.05,
    weight_decay=5e-4,
    num_workers=2,
    ...
    # normalize to [-1,1]-style (mean=0.5,std=0.5)
    ...
    full_train = datasets.CIFAR10(root=data_root, train=True, download=True, transform=tf_train)
    _ = datasets.CIFAR10(root=data_root, train=False, download=True, transform=tf_test)
    ...
    query_idx = idx[n_train:]

    train_set = Subset(full_train, train_idx)
    query_set = Subset(full_train, query_idx)

    def _train_one(model, ds, epochs, name="model"):
        model = model.to(device)
        model.train()

        loader = DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
        opt = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay, nesterov=True)
        sch = optim.lr_scheduler.MultiStepLR(opt, milestones=[int(0.6*epochs), int(0.85*epochs)], gamma=0.2)
        ...
        for ep in range(1, epochs + 1):
            ...
            sch.step()
            print(f"[{name}] epoch {ep:03d}/{epochs} | loss={loss_sum/total:.4f} | acc={correct/total:.4f}")

        model.eval()
        return model

    target_model = build_vgg16_cifar(num_classes=10)
    target_model = _train_one(target_model, train_set, train_epochs_target, name="target(VGG16)")

    shadow_model = build_vgg16_cifar(num_classes=10)
    shadow_model = _train_one(shadow_model, query_set, train_epochs_shadow, name="shadow(VGG16)")

    return target_model, shadow_model, train_set, query_set


# ============================================================
# 3) Fallback cVAE (only used if your train_cVAE import fails)
# ============================================================
class SimpleConvCVAE(nn.Module):
    def __init__(self, z_dim=128, num_classes=10, img_ch=3):
        super().__init__()
        self.z_dim = z_dim
        self.num_classes = num_classes
        self.emb = nn.Embedding(num_classes, 32)

        self.enc = nn.Sequential(
            nn.Conv2d(img_ch, 64, 4, 2, 1),   # 32->16
            nn.ReLU(True),
        ...
        self.fc_mu = nn.Linear(512*2*2 + 32, z_dim)
        self.fc_lv = nn.Linear(512*2*2 + 32, z_dim)

        self.fc_dec = nn.Linear(z_dim + 32, 512*2*2)
        self.dec = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(64, img_ch, 4, 2, 1),# 16->32
            nn.Tanh(),  # [-1,1]
        )
    ...
    def decode(self, z, y):
        ...
        h = self.fc_dec(h).view(z.size(0), 512, 2, 2)
        return self.dec(h)
...
def train_cvae_fallback(query_set, device="cuda", z_dim=128, epochs=40, batch_size=128, lr=2e-4):
    loader = DataLoader(query_set, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True)
    vae = SimpleConvCVAE(z_dim=z_dim, num_classes=10).to(device)
    ...
    for ep in range(1, epochs + 1):
        ...
        print(f"[cVAE(fallback)] epoch {ep:03d}/{epochs} | loss={loss_sum/total:.4f}")
...
# ============================================================
# 4) MI imports (use your project if available; otherwise fallback proxy)
# ============================================================
try:
    ...
    print("[WARN] compute_information_plane import failed:", repr(e))

    @torch.no_grad()
    def compute_MI_full_dataset(model, dataset, device="cuda"):
        model.eval()
        ...
        p = np.bincount(pred, minlength=10).astype(np.float64)
        p = p / (p.sum() + 1e-12)
        Hpred = -np.sum(p * np.log(p + 1e-12))

        joint = np.zeros((10, 10), dtype=np.float64)
        ...
        mask = joint > 0
        I = np.sum(joint[mask] * (np.log(joint[mask] + 1e-12) - np.log((pa @ pb)[mask] + 1e-12)))
        return float(Hpred), float(I)

    @torch.no_grad()
    def compute_dataset_MI_balanced_with_MIcal(model, dataset, device="cuda", samples_per_class=1500, num_points=40):
        model.eval()
        labels = []
        ...
        for _ in range(num_points):
            ...
            for c in range(10):
                ...
                if take > 0:
                    pick = np.random.choice(idx, size=take, replace=False)
                    sel.append(pick)
            if len(sel) == 0:
                out_xt.append(0.0); out_ty.append(0.0); continue
            sel = np.concatenate(sel, axis=0)
            ds = Subset(dataset, sel.tolist())
            xt, ty = compute_MI_full_dataset(model, ds, device=device)
            out_xt.append(xt); out_ty.append(ty)
        return out_xt, out_ty


# ============================================================
# 5) Estimate per-class prior r_y(t) (tau-aligned)
# ============================================================
@torch.no_grad()
...
# ============================================================
# 6) Budget C estimation on shadow data
# ============================================================
@torch.no_grad()
...
# ============================================================
# 8) Inversion core (z = SGLD, theta = Adam with NES dL/dx)
# ============================================================
def ib_sgld_blackbox_nes_multichain(
    ...
    for t in range(total_steps):
        # λ not forced to 0 during burn-in
...
# ============================================================
# 10) Dual-ascent calibration of lambda on SHADOW
# ============================================================
def calibrate_lambda_dual_on_shadow(
    ...
    for it in range(max_iters):
        ...
        print(f"  iter={it:02d} | λ={lam:.6f} | mean_conf={mean_conf:.4f} | mean_KL={mean_kl:.6f} | rel_err={rel_err:.3f}")
...
def main():
    ...
    # -------------------------
    # 1) Train target/shadow (VGG16) + split CIFAR-10 (80/20)
    # -------------------------
    target_model, shadow_model, train_set, query_set = train_target_model_vgg16_cifar10(
        device=device,
        seed=0,
        train_epochs_target=100,
        train_epochs_shadow=100,
        batch_size=128,
        lr=0.05,
    )
    target_model.eval()
    ...
    # -------------------------
    # 2) Train generator on query_set
    # -------------------------
    generator = None
    try:
        from train_cVAE import train_cVAE  # your project module (if exists)
        generator = train_cVAE(query_set, device=device)
        print("[OK] Using your project train_cVAE.")
    except Exception as e:
        print("[WARN] train_cVAE import/call failed, using fallback cVAE:", repr(e))
        generator = train_cvae_fallback(query_set, device=device, z_dim=128, epochs=40)

    # -------------------------
    # 3) Estimate shadow prior r_y(t)
    # -------------------------
    print("\n📌 Estimating r_y(t) from SHADOW(VGG16) on query_set (tau-aligned) ...")
    ...
    # -------------------------
    # 5) (B2) Calibrate lambda*
    # -------------------------
    eval_subset = list(range(10))  # CIFAR-10 => 0..9
    ...
    ensure_dir("results")
    with open("results/methodB_lambda_dual_history_cifar10_vgg16.txt", "w") as f:
        for r in hist:
            f.write(str(r) + "\n")
    ...
    imgs_gen, labels_gen = run_inversion_experiment(
        ...
        device=device,
        save_dir="results/blackbox_methodB_cifar10_vgg16",
        z_dim=128,
        total_steps=3500,
        burn_in=800,
        num_chains=64,
    ...
    # -------------------------
    # 7) Information Plane + axis-aligned ellipse
    # -------------------------
    print("\n📊 Computing TRAIN MI ...")
    ...
    ax.add_patch(ell)
    ax.scatter([cx], [cy], marker="x", s=80, label=f"Gen mean center (a={a:.4f}, b={b:.4f})")

    # expand limits to show ellipse
    ...
    ax.set_xlabel("I(X;T)")
    ax.set_ylabel("I(T;Y)")
    ax.set_title("Information Plane: CIFAR-10 (VGG16 target) | Train vs Query vs Generated")
    ax.grid(True)
    ax.legend()

    out_png = "results/info_plane_methodB_cifar10_vgg16_with_gen_ellipse_axisaligned.png"
